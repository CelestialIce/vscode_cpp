...
#include <cstring>
#include <iostream>
#include <vector> // Add the missing header for vector
using namespace std;
...
bool dfs(int x, int y) {
    ...
    for (int t = 1; t <= 9; t++) { // Fix the loop condition
        if (!st_row[x][t] && !st_col[y][t] && !st_block[x / 3][y / 3][t]) {
            st_row[x][t] = st_col[y][t] = st_block[x / 3][y / 3][t] = true; // Fix the assignment
            g[x][y] = t;
            if (dfs(x, y + 1)) return true;
            g[x][y] = 0;
            st_row[x][t] = st_col[y][t] = st_block[x / 3][y / 3][t] = false; // Fix the assignment
        }
    }
    return false;
}
